...
class Reservation extends Model
{
    ...
    public static function putReservation($users_id,Request $request)
    {
        $newReservationInfomation = [
            'date' => $request -> new_date,
            'time' => $request -> new_time,
            'num_users' => $request -> new_num_users
        ];
        $isCheckedReservation = Reservation::where('user_id',$users_id) ->where('id',$request -> id) -> first();
        if($isCheckedReservation){
            $isCheckedReservation -> fill($newReservationInfomation) -> save();
            return $isCheckedReservation;
        }else{
            return false;
        }
    }
